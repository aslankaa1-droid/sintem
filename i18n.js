...
(function (root) {
  ...
  const I18N = {
    ru: {
      ...
      'hero.note':    '5 запросов на каждого помощника, без карты, без срока годности.',

      'agents.h2':   'Что внутри. Пять помощников',
      'agents.lead': 'Каждый помощник = одна задача, которую вы сейчас делаете руками. Все пять — в одной подписке, без отдельных оплат и интеграций.',
      'ba.before':   'До',
      'ba.after':    'После',
      ...
      'price.h2':   'Тарифы',
      'price.lead': 'В каждом тарифе все 5 помощников без ограничений по типам. Лимит общий по запросам в месяц. Триал: 5 запросов на каждого помощника бесплатно, без карты, без срока годности.',
      'p1.h':   'Старт',
      'p1.for': 'Один селлер, до 30 SKU, тестирует подход.',
      'p1.f1':  '200 запросов в месяц',
      'p1.f2':  'Все 5 помощников',
      ...
      'p2.f2':  'Все 5 помощников без ограничений',
      ...
      'faq.a6': 'Да. Все 5 помощников работают и с WB, и с Ozon. Юнит-экономика учитывает разные комиссионные сетки обеих площадок. Скаут конкурентов работает по обеим выдачам.',
      'faq.q7': 'Можно ли обучить бота под мой бренд и тон?',
      'faq.a7': 'Да, для помощника «Ответ на отзывы». Командой /style один раз присылаете 5–10 примеров своих ответов клиентам, бот запоминает голос бренда и дальше пишет в нём. Перенастроить можно когда угодно.',
      'faq.q8': 'Какая поддержка?',
      'faq.a8': 'Поддержка в Telegram, @sintem_support. Время ответа в рабочие дни — до 4 часов. Сложные кейсы (баг в расчётах, спорный ответ помощника) разбирает лично основатель.',
      ...
      'priv.in_l3':   'Ответы бота, сгенерированные AI-помощниками.',
      ...
      'priv.ck':      'Сайт <code>sintem.ru</code> не использует cookies для трекинга и не подключает внешние аналитические системы (Google Analytics, Yandex.Metrica и т.п.). Браузер запоминает только выбранный вами язык интерфейса (localStorage, ключ <code>sintem.lang</code>).',
    ...
    en: {
      ...
      'priv.ck':      'The <code>sintem.ru</code> website does not use cookies for tracking and does not load external analytics (Google Analytics, Yandex.Metrica, etc.). The browser only remembers your interface-language choice (localStorage, key <code>sintem.lang</code>).',
    ...
    fr: {
      ...
      'priv.ck':      'Le site <code>sintem.ru</code> n’utilise pas de cookies de tracking et n’intègre pas d’analytique externe (Google Analytics, Yandex.Metrica, etc.). Le navigateur ne mémorise que la langue d’interface choisie (localStorage, clé <code>sintem.lang</code>).',
    ...
    ar: {
      ...
      'priv.ck':      'موقع <code>sintem.ru</code> لا يستخدم كوكيز التتبّع ولا يُحمِّل خدمات تحليل خارجية (Google Analytics, Yandex.Metrica, إلخ). يحفظ المتصفح فقط لغة الواجهة المُختارة (localStorage، المفتاح <code>sintem.lang</code>).',
      'priv.h2_rt':   'حقوقك',
  ...
  // ПУБЛИЧНО АКТИВНЫЕ языки. EN/FR/AR временно скрыты до первых платящих в РФ
  // (мотивация — отчёт экспертизы 2026-05-11: «4 языка до 10 платящих = premature optimization»).
  // Чтобы вернуть — расширить список + раскомментировать кнопки в *.html шапках.
  const PUBLIC_LANGS = ['ru'];

  function detect() {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      if (saved && I18N[saved] && PUBLIC_LANGS.includes(saved)) return saved;
    } catch (e) { /* private mode */ }
    const nav = (navigator.language || 'ru').toLowerCase();
    if (PUBLIC_LANGS.includes('en') && nav.startsWith('en')) return 'en';
    if (PUBLIC_LANGS.includes('fr') && nav.startsWith('fr')) return 'fr';
    if (PUBLIC_LANGS.includes('ar') && nav.startsWith('ar')) return 'ar';
    return 'ru';
  }
  ...
  function apply(lang) {
    // Guard: если выбранный язык скрыт (EN/FR/AR временно отключены), откатываемся на ru.
    if (!PUBLIC_LANGS.includes(lang)) lang = 'ru';
  ...
  root.SINTEM_I18N = {
    apply: apply,
    detect: detect,
    // languages: только публично активные. Полный набор словарей — Object.keys(I18N).
    languages: PUBLIC_LANGS.slice(),
